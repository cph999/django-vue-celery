...
import os
from backend.spider.models import BookData, Comment, TaskInfo
...
results = []
head_data = ["书名", "作者", "SPU", "价格", "优惠信息", "店铺"]


@shared_task
def jdSpider(taskInfo, type):
    comment_len = 10
    os.environ['NO_PROXY'] = 'search.jd.com,jd.com'
    headers = {
    ...
    for book_info in book_container.select("ul > li > div"):
        ...
        book_result.append(tips)
        book_result.append(outlet_name)
        book = BookData(book_name=book_name, book_tips=tips, book_price=book_price, book_author=author_name,
                        book_outlet=outlet_name, book_spu_number=detail_number)
        book.save()
